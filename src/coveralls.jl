...
module Coveralls
    ...
    function prepare_request(fcs::Vector{FileCoverage}, local_env::Bool, git_info=query_git_info)
        ...
        elseif haskey(ENV, "GITHUB_ACTION")
            ...
            event = open(JSON.Parser.parse, ENV["GITHUB_EVENT_PATH"])
            github_pr_info = get(event, "pull_request", Dict())

            github_branch = get(get(github_pr_info, "head", Dict()), "ref", "")
            github_sha    = get(get(github_pr_info, "head", Dict()), "sha", "")
